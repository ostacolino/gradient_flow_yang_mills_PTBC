	...
	for(i0 = 0; i0 < NCOLOR; i0++)
		...
		for(i1 = 0; i1 < NCOLOR; i1++)
			...
			for(i2 = 0; i2 < NCOLOR; i2++)
				{
				for(i3 = 0; i3 < NCOLOR; i3++) { A->comp[i0][i1][i2][i3] = 0.0 + 0.0 * I; }
				}
			}
		}
	}
...
inline void one_TensProd(TensProd *restrict A)
	...
	for(i0 = 0; i0 < NCOLOR; i0++)
		{
		for(i1 = 0; i1 < NCOLOR; i1++) { A->comp[i0][i0][i1][i1] = 1.0 + 0.0 * I; }
		}
	}
...
inline void equal_TensProd(TensProd *restrict A, TensProd const *const restrict B)
	...
	for(i0 = 0; i0 < NCOLOR; i0++)
		...
		for(i1 = 0; i1 < NCOLOR; i1++)
			...
			for(i2 = 0; i2 < NCOLOR; i2++)
				{
				for(i3 = 0; i3 < NCOLOR; i3++) { A->comp[i0][i1][i2][i3] = B->comp[i0][i1][i2][i3]; }
				}
			}
		}
	}
...
inline void times_equal_real_TensProd(TensProd *restrict A, double r)
	...
	for(i0 = 0; i0 < NCOLOR; i0++)
		...
		for(i1 = 0; i1 < NCOLOR; i1++)
			...
			for(i2 = 0; i2 < NCOLOR; i2++)
				{
				for(i3 = 0; i3 < NCOLOR; i3++) { A->comp[i0][i1][i2][i3] *= r; }
				}
			}
		}
	}
...
// A=B*C
inline void times_TensProd(TensProd *restrict A, TensProd const *const restrict B, TensProd const *const restrict C)
	{
	#ifdef DEBUG
	if(A == B || A == C || B == C)
		{
		...
		for(j = 0; j < NCOLOR; j++)
			...
			for(k = 0; k < NCOLOR; k++)
				...
				for(l = 0; l < NCOLOR; l++)
					...
					for(a = 0; a < NCOLOR; a++)
						{
						for(b = 0; b < NCOLOR; b++) { sum += B->comp[i][a][k][b] * C->comp[a][j][b][l]; }
						}
					A->comp[i][j][k][l] = sum;
					}
				}
...
inline double retr_TensProd(TensProd const *const restrict A)
	...
	for(i0 = 0; i0 < NCOLOR; i0++)
		{
		for(i1 = 0; i1 < NCOLOR; i1++) { tr += A->comp[i0][i0][i1][i1]; }
		}
	ris = creal(tr);
	ris /= (NCOLOR * NCOLOR);
