...
inline void one_SuN(SuN *restrict A)
	...
	for(i = 0; i < NCOLOR * NCOLOR; i++) { A->comp[i] = 0.0 + 0.0 * I; }

	for(i = 0; i < NCOLOR; i++) { A->comp[m(i, i)] = 1.0 + 0.0 * I; }
	}
...
inline void equal_SuN(SuN *restrict A, SuN const *const restrict B)
	...
	for(i = 0; i < NCOLOR * NCOLOR; i++) { A->comp[i] = B->comp[i]; }
	}
...
inline void equal_dag_SuN(SuN *restrict A, SuN const *const restrict B)
	...
	for(i = 0; i < NCOLOR; i++)
		{
		for(j = 0; j < NCOLOR; j++) { A->comp[m(i, j)] = conj(B->comp[m(j, i)]); }
		}
	}
...
inline void minus_equal_times_real_SuN(SuN *restrict A, SuN const *const restrict B, double r)
	...
	for(i = 0; i < NCOLOR * NCOLOR; i++) { A->comp[i] -= (r * B->comp[i]); }
	}
...
inline void lin_comb_SuN(SuN *restrict A, double b, SuN const *const restrict B, double c, SuN const *const restrict C)
	...
	for(i = 0; i < NCOLOR * NCOLOR; i++) { A->comp[i] = b * (B->comp[i]) + c * (C->comp[i]); }
	}
...
inline void lin_comb_dag1_SuN(SuN *restrict A, double b, SuN const *const restrict B, double c, SuN const *const restrict C)
	...
	for(i = 0; i < NCOLOR; i++)
		{
		for(j = 0; j < NCOLOR; j++) { A->comp[m(i, j)] = b * conj(B->comp[m(j, i)]) + c * (C->comp[m(i, j)]); }
		}
	}

// A=b*B+c*C^{dag}
inline void lin_comb_dag2_SuN(SuN *restrict A, double b, SuN const *const restrict B, double c, SuN const *const restrict C)
	{
	#ifdef DEBUG
	if(A == B || A == C || B == C)
		{
	...
	for(i = 0; i < NCOLOR; i++)
		{
		for(j = 0; j < NCOLOR; j++) { A->comp[m(i, j)] = b * (B->comp[m(i, j)]) + c * conj(C->comp[m(j, i)]); }
		}
	}

// A=b*B^{dag}+c*C^{dag}
inline void lin_comb_dag12_SuN(SuN *restrict A, double b, SuN const *const restrict B, double c, SuN const *const restrict C)
	{
	#ifdef DEBUG
	if(A == B || A == C || B == C)
		{
	...
	for(i = 0; i < NCOLOR; i++)
		{
		for(j = 0; j < NCOLOR; j++) { A->comp[m(i, j)] = b * conj(B->comp[m(j, i)]) + c * conj(C->comp[m(j, i)]); }
		}
	}
...
inline void times_equal_real_SuN(SuN *restrict A, double r)
	...
	for(i = 0; i < NCOLOR * NCOLOR; i++) { A->comp[i] *= r; }
	}
...
inline void times_equal_SuN(SuN *restrict A, SuN const *const restrict B)
	...
	for(i = 0; i < NCOLOR; i++)
		{
		for(j = 0; j < NCOLOR; j++) { aux[j] = A->comp[m(i, j)]; }

		for(j = 0; j < NCOLOR; j++)
			{
			sum = 0.0 + 0.0 * I;
			for(k = 0; k < NCOLOR; k++) { sum += aux[k] * (B->comp[m(k, j)]); }
			A->comp[m(i, j)] = sum;
			}
		}
	}
...
inline void times_equal_dag_SuN(SuN *restrict A, SuN const *const restrict B)
	...
	for(i = 0; i < NCOLOR; i++)
		...
		for(j = 0; j < NCOLOR; j++)
			{
			sum = 0.0 + 0.0 * I;
			for(k = 0; k < NCOLOR; k++) { sum += aux[k] * conj(B->comp[m(j, k)]); }
			A->comp[m(i, j)] = sum;
			}
		}
	}
...
inline void times_SuN(SuN *restrict A, SuN const *const restrict B, SuN const *const restrict C)
	...
	for(i = 0; i < NCOLOR; i++)
		...
		for(j = 0; j < NCOLOR; j++)
			{
			sum = 0.0 + 0.0 * I;
			for(k = 0; k < NCOLOR; k++) { sum += (B->comp[m(i, k)]) * (C->comp[m(k, j)]); }
			A->comp[m(i, j)] = sum;
			}
		}
	}
...
inline void times_dag1_SuN(SuN *restrict A, SuN const *const restrict B, SuN const *const restrict C)
	...
	for(i = 0; i < NCOLOR; i++)
		...
		for(j = 0; j < NCOLOR; j++)
			{
			sum = 0.0 + 0.0 * I;
			for(k = 0; k < NCOLOR; k++) { sum += conj(B->comp[m(k, i)]) * (C->comp[m(k, j)]); }
			A->comp[m(i, j)] = sum;
			}
		}
	}
...
inline void times_dag2_SuN(SuN *restrict A, SuN const *const restrict B, SuN const *const restrict C)
	...
	for(i = 0; i < NCOLOR; i++)
		...
		for(j = 0; j < NCOLOR; j++)
			{
			sum = 0.0 + 0.0 * I;
			for(k = 0; k < NCOLOR; k++) { sum += (B->comp[m(i, k)]) * conj(C->comp[m(j, k)]); }
			A->comp[m(i, j)] = sum;
			}
		}
	}
...
inline void times_dag12_SuN(SuN *restrict A, SuN const *const restrict B, SuN const *const restrict C)
	...
	for(i = 0; i < NCOLOR; i++)
		...
		for(j = 0; j < NCOLOR; j++)
			{
			sum = 0.0 + 0.0 * I;
			for(k = 0; k < NCOLOR; k++) { sum += conj(B->comp[m(k, i)]) * conj(C->comp[m(j, k)]); }
			A->comp[m(i, j)] = sum;
			}
		}
	}
...
inline double retr_SuN(SuN const *const restrict A)
	...
	tr = 0.0 + 0.0 * I;
	for(i = 0; i < NCOLOR; i++) { tr += A->comp[m(i, i)]; }
...
inline complex double det_SuN(SuN const *const restrict A)
	...
	if(i > 0) { ris = 1.0 + 0.0 * I; }
	else
		{
		ris = -1.0 + 0.0 * I;
		}

	for(i = 0; i < NCOLOR; i++) { ris *= (lu.comp[m(i, i)]); }
...
inline void ta_SuN(SuN *restrict A)
	...
	for(i = 1; i < NCOLOR; i++) { trace += aux.comp[m(i, i)]; }
	trace /= (double)NCOLOR;

	for(i = 0; i < NCOLOR; i++) { aux.comp[m(i, i)] -= trace; }
...
inline int ta_check_SuN(SuN const *const restrict A)
	...
	for(i = 0; i < NCOLOR; i++)
		{
		for(j = 0; j < NCOLOR; j++) { aux += (A->comp[m(i, j)] + conj(A->comp[m(j, i)])); }
		}
	if(cabs(aux) > MIN_VALUE) ris = 1;

	aux = 0.0;
	for(i = 0; i < NCOLOR; i++) { aux += A->comp[m(i, i)]; }
	if(cabs(aux) > MIN_VALUE) ris = 1;
...
inline void TensProd_init_SuN(TensProd *restrict TP, SuN const *const restrict A1, SuN const *const restrict A2)
	...
	for(i = 0; i < NCOLOR; i++)
		...
		for(j = 0; j < NCOLOR; j++)
			...
			for(k = 0; k < NCOLOR; k++)
				{
				for(l = 0; l < NCOLOR; l++) { TP->comp[i][j][k][l] = conj(A1->comp[m(i, j)]) * A2->comp[m(k, l)]; }
				}
			}
		}
	}
