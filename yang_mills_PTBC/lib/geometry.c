...
void init_geometry(Geometry *geo, GParam const *const param)
	...
	for(r = 0; r < param->d_volume; r++)
		...
		for(i = 0; i < STDIM; i++)
			...
			valuep = value + 1;
			if(valuep >= param->d_size[i]) { valuep -= param->d_size[i]; }
			cartcoord[i] = valuep;
			rp = cart_to_si(cartcoord, param);
			geo->d_nnp[r][i] = rp;

			valuem = value - 1;
			if(valuem < 0) { valuem += param->d_size[i]; }
			cartcoord[i] = valuem;
...
void free_geometry(Geometry *geo, GParam const *const param)
	...
	for(r = 0; r < param->d_size[0]; r++) { free(geo->d_tsp[r]); }
	free(geo->d_tsp);
	}
...
void lex_to_cart(int *cartcoord, long lex, GParam const *const param)
	...
	aux[0] = 1;
	for(i = 1; i < STDIM; i++) { aux[i] = aux[i - 1] * param->d_size[i - 1]; }
	// aux[0]=1
	// aux[1]=size[0]
	// aux[2]=size[0]*size[1]
	// ...
...
long cart_to_lexeo(int const *const cartcoord, GParam const *const param)
	...
	eo = 0;
	for(i = 0; i < STDIM; i++) { eo += cartcoord[i]; }

	if(eo % 2 == 0) { return lex / 2; }
	else
		{
...
void lexeo_to_cart(int *cartcoord, long lexeo, GParam const *const param)
	...
	if(param->d_volume % 2 == 0)
		{
		if(lexeo < param->d_volume / 2) { lex = 2 * lexeo; }
		else
			{
		...
		if((eo == 0 && lexeo >= param->d_volume / 2) || (eo == 1 && lexeo < param->d_volume / 2))
			{
			lex += 1;
			lex_to_cart(cartcoord, lex, param);
			}
...
void lexsp_to_cartsp(int *ccsp, long lexsp, GParam const *const param)
	...
	aux[0] = 1;
	for(i = 1; i < STDIM - 1; i++) { aux[i] = aux[i - 1] * param->d_size[i]; }
	// aux[0]=1
	// aux[1]=size[1]
	// aux[2]=size[1]*size[2]
	// ...
...
long cartsp_to_lexeosp(int const *const ccsp, GParam const *const param)
	...
	eo = 0;
	for(i = 0; i < STDIM - 1; i++) { eo += ccsp[i]; }

	if(eo % 2 == 0) { return lexsp / 2; }
	else
		{
...
void lexeosp_to_cartsp(int *ccsp, long lexeosp, GParam const *const param)
	...
	if(param->d_space_vol % 2 == 0)
		{
		if(lexeosp < param->d_space_vol / 2) { lexsp = 2 * lexeosp; }
		else
			{
		...
		if((eo == 0 && lexeosp >= param->d_space_vol / 2) || (eo == 1 && lexeosp < param->d_space_vol / 2))
			{
			lexsp += 1;
			lexsp_to_cartsp(ccsp, lexsp, param);
			}
...
void lexeo_to_lexeosp_and_t(long *lexeosp, int *t, long lexeo, GParam const *const param)
	...
	*t = cc[0];
	...
	for(i = 0; i < STDIM - 1; i++) { ccsp[i] = cc[i + 1]; }
		...
		for(coord[1] = size_min[1]; coord[1] < size_max[1]; coord[1]++)
			...
			for(coord[2] = size_min[2]; coord[2] < size_max[2]; coord[2]++)
				...
				for(coord[3] = size_min[3]; coord[3] < size_max[3]; coord[3]++)
					...
					for(i = 0; i < STDIM; i++) real_coord[i] = periodic_condition(coord[i], param->d_size[i]);
...
void free_rect(Rectangle *most_update) { free(most_update->rect_sites); }
...
void init_rect_hierarc(Rectangle **most_update, Rectangle **clover_rect, GParam const *const param)
	...
	if(param->d_N_hierarc_levels == 0)
		...
		for(i = 0; i < param->d_N_hierarc_levels; i++) { init_rect(&((*most_update)[i]), param->d_L_rect[i], param); }
		...
		for(i = 0; i < param->d_N_hierarc_levels; i++) init_rect(&((*clover_rect)[i]), 2 + param->d_L_rect[i], param);
		#else
		clover_rect = NULL;
...
void free_rect_hierarc(Rectangle *most_update, Rectangle *clover_rect, GParam const *const param)
	...
	if(param->d_N_hierarc_levels == 0)
		...
		{
		int i;
		for(i = 0; i < param->d_N_hierarc_levels; i++) free_rect(&(most_update[i]));
		free(most_update);
		#ifdef THETA_MODE
		for(i = 0; i < param->d_N_hierarc_levels; i++) free_rect(&(clover_rect[i]));
		free(clover_rect);
		#else
