...
void FT_topcharge_timeslices(Gauge_Conf const * const GC,
                 ...
                 GParam const * const param, double complex *ris, double time, FILE *topchar_MOM_tprof_filep){
   if(!(STDIM==4 && NCOLOR>1) && !(STDIM==2 && NCOLOR==1) )
   {
      fprintf(stderr, "Wrong number of dimensions or number of colors! (%s, %d)\n", __FILE__, __LINE__);
      exit(EXIT_FAILURE);
   ...
   for(int kappa = 0; kappa<1; kappa++){       
	 ...
	 si_to_cart(coordinate,r,param);
      ...
      fprintf(topchar_MOM_tprof_filep, "%ld %.12g %d ", GC->update_index, time, kappa);
   ...
   int err=posix_memalign((void**)&zp, (size_t)DOUBLE_ALIGN, (size_t)N_s/2 * sizeof(double complex*));
   if (err!=0){
        fprintf(stderr, "Problems in allocating a vector (%s, %d)\n", __FILE__, __LINE__);
        exit(EXIT_FAILURE);
        }
   for(int i = 0; i<N_s/2;i++){
      err=posix_memalign((void**)&zp[i], (size_t)DOUBLE_ALIGN, (size_t)N_t*sizeof(double complex));
      if (err!=0){
        fprintf(stderr, "Problems in allocating a vector (%s, %d)\n", __FILE__, __LINE__);
        exit(EXIT_FAILURE);
        }
...
void topo_obs_cooling(Gauge_Conf const *const GC, Geometry const *const geo, GParam const *const param, double *charge, double *chi_prime, double *meanplaq, double *sum_q_timeslices, FILE *topchar_tprof_filep)
	...
	if(param->d_coolsteps > 0) // if using cooling
		...
		{
		double ris, ris2=0.0, plaqs, plaqt;
		int iter;
		...
		for(iter = 0; iter < (param->d_coolrepeat); iter++)
			{
			charge[iter] = ris;
			if(param->d_chi_prime_meas == 1) chi_prime[iter] = ris2;
			#if(STDIM == 4)
			meanplaq[iter] = 0.5 * (plaqs + plaqt);
			#else
			meanplaq[iter] = plaqt;
...
void perform_measures_localobs_with_adaptive_gradflow(Gauge_Conf *GC, Geometry const *const geo, GParam const *const param, FILE *datafilep, FILE *chiprimefilep, FILE *topchar_tprof_filep, FILE *topchar_MOM_tprof_filep)
	{
	#if((STDIM == 4 && NCOLOR > 1) || (STDIM == 2 && NCOLOR == 1))
	int gradflowrepeat;
