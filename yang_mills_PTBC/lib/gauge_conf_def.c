...
void init_gauge_conf(Gauge_Conf *GC, GParam const *const param) { init_gauge_conf_from_file_with_name(GC, param, param->d_conf_file); }
...
void init_bound_cond(Gauge_Conf *GC, GParam const *const param, int const i)
	...
	for(r = 0; r < param->d_volume; r++)
		for(j = 0; j < STDIM; j++) { GC->C[r][j] = 1.0; }
	...
	if(param->d_N_replica_pt > 1)
		...
		for(r = 0; r < param->d_volume; r++)
			...
			if((cartcoord[param->d_defect_dir] == ((param->d_size[param->d_defect_dir]) - 1)) && (cartcoord[perp_dir[param->d_defect_dir][0]] < (param->d_L_defect[0])) && (cartcoord[perp_dir[param->d_defect_dir][1]] < (param->d_L_defect[1])) && (cartcoord[perp_dir[param->d_defect_dir][2]] < (param->d_L_defect[2]))) is_on_defect = TRUE;
			else
				is_on_defect = FALSE;
...
void free_gauge_conf(Gauge_Conf *GC, GParam const *const param)
	...
	for(i = 0; i < (param->d_volume); i++) { free(GC->lattice[i]); }
	free(GC->lattice);
...
void free_bound_cond(Gauge_Conf *GC, GParam const *const param)
	{
	long r;
	for(r = 0; r < (param->d_volume); r++) { free(GC->C[r]); }
	free(GC->C);
	}
...
void write_conf_on_file_with_name(Gauge_Conf const *const GC, GParam const *const param, char const *const namefile)
	...
	if(fp == NULL)
		...
		{
		fprintf(fp, "%d ", STDIM);
		for(i = 0; i < STDIM; i++) { fprintf(fp, "%d ", param->d_size[i]); }
	...
	if(fp == NULL)
		...
		for(lex = 0; lex < param->d_volume; lex++)
			{
			si = lex_to_si(lex, param);
			for(mu = 0; mu < STDIM; mu++) { print_on_binary_file_bigen(fp, &(GC->lattice[si][mu])); }
			}
		fclose(fp);
		}
	}

void write_conf_on_file(Gauge_Conf const *const GC, GParam const *const param) { write_conf_on_file_with_name(GC, param, param->d_conf_file); }
...
void write_replica_on_file_back(Gauge_Conf const *const GC, GParam const *const param)
	...
	for(i = 0; i < param->d_N_replica_pt; i++)
		...
		if(counter == 0) sprintf(aux_back, "_back0");
		else
...
void write_conf_on_file_back(Gauge_Conf const *const GC, GParam const *const param)
	...
	if(counter == 0) { sprintf(aux, "_back0"); }
	else
		{
		sprintf(aux, "_back1");
		}
...
void init_gauge_conf_from_gauge_conf(Gauge_Conf *GC, Gauge_Conf const *const GC2, GParam const *const param)
	...
	for(r = 0; r < (param->d_volume); r++)
		{
		for(mu = 0; mu < STDIM; mu++) { equal(&(GC->lattice[r][mu]), &(GC2->lattice[r][mu])); }
		}
...
void compute_md5sum_conf(char *res, Gauge_Conf const *const GC, GParam const *const param)
	...
	for(lex = 0; lex < param->d_volume; lex++)
		...
		for(mu = 0; mu < STDIM; mu++)
			...
			#elif NCOLOR == 2
			for(k = 0; k < 4; k++) { MD5_Update(&mdContext, &(matrix.comp[k]), sizeof(double)); }
			#else
			for(k = 0; k < NCOLOR * NCOLOR; k++) { MD5_Update(&mdContext, &(matrix.comp[k]), sizeof(double complex)); }
			#endif
			}
		}
	MD5_Final(c, &mdContext);

	for(k = 0; k < MD5_DIGEST_LENGTH; k++) { sprintf(&(res[2 * k]), "%02x", c[k]); }
	#else
...
void free_polycorr_stuff(Gauge_Conf *GC, GParam const *const param)
	...
	for(i = 0; i < NLEVELS; i++)
		{
		for(j = 0; j < (param->d_size[0] / param->d_ml_step[i]); j++) { free(GC->ml_polycorr[i][j]); }
		free(GC->ml_polycorr[i]);
		}
	free(GC->ml_polycorr);

	for(i = 0; i < param->d_size[0] / param->d_ml_step[NLEVELS - 1]; i++) { free(GC->loc_poly[i]); }
	free(GC->loc_poly);
	}
...
void write_polycorr_on_file(Gauge_Conf const *const GC, GParam const *const param, int iteration)
	...
	if(fp == NULL)
		...
		for(j = 0; j < param->d_size[0] / param->d_ml_step[0]; j++)
			{
			for(i = 0; i < (param->d_space_vol); i++) { print_on_binary_file_bigen_TensProd(fp, &(GC->ml_polycorr[0][j][i])); }
			}
...
void read_polycorr_from_file(Gauge_Conf const *const GC, GParam const *const param, int *iteration)
	...
	if(fp == NULL)
		...
		for(j = 0; j < param->d_size[0] / param->d_ml_step[0]; j++)
			{
			for(i = 0; i < (param->d_space_vol); i++) { read_from_binary_file_bigen_TensProd(fp, &(GC->ml_polycorr[0][j][i])); }
			}
...
void compute_md5sum_polycorr(char *res, Gauge_Conf const *const GC, GParam const *const param)
			...
			for(n1 = 0; n1 < NCOLOR; n1++)
				...
				for(n2 = 0; n2 < NCOLOR; n2++)
					...
					for(n3 = 0; n3 < NCOLOR; n3++)
						{
						for(n4 = 0; n4 < NCOLOR; n4++) { MD5_Update(&mdContext, &((GC->ml_polycorr[0][j][i]).comp[n1][n2][n3][n4]), sizeof(double complex)); }
						}
					}
				}
			}
	...
	for(i = 0; i < MD5_DIGEST_LENGTH; i++) { sprintf(&(res[2 * i]), "%02x", c[i]); }
	#else
	// just to avoid warning at compile time
	(void)res;
	(void)GC;
...
void free_polycorradj(Gauge_Conf *GC, GParam const *const param)
	...
	for(i = 0; i < NLEVELS; i++)
		{
		for(j = 0; j < (param->d_size[0] / param->d_ml_step[i]); j++) { free(GC->ml_polycorradj[i][j]); }
...
void write_tube_conn_stuff_on_file(Gauge_Conf const *const GC, GParam const *const param, int iteration)
	...
	if(fp == NULL)
		...
		for(i = 0; i < (param->d_space_vol); i++) { print_on_binary_file_bigen_TensProd(fp, &(GC->ml_polyplaq[0][i])); }
		for(i = 0; i < (param->d_space_vol); i++) { print_on_binary_file_bigen_TensProd(fp, &(GC->ml_polyplaqconn[0][i])); }
...
void read_tube_conn_stuff_from_file(Gauge_Conf const *const GC, GParam const *const param, int *iteration)
	...
	if(fp == NULL)
		...
		for(i = 0; i < (param->d_space_vol); i++) { read_from_binary_file_bigen_TensProd(fp, &(GC->ml_polyplaq[0][i])); }
		for(i = 0; i < (param->d_space_vol); i++) { read_from_binary_file_bigen_TensProd(fp, &(GC->ml_polyplaqconn[0][i])); }
	...
	for(i = 0; i < (param->d_space_vol); i++)
		...
		for(n1 = 0; n1 < NCOLOR; n1++)
			...
			for(n2 = 0; n2 < NCOLOR; n2++)
				...
				for(n3 = 0; n3 < NCOLOR; n3++)
					{
					for(n4 = 0; n4 < NCOLOR; n4++) { MD5_Update(&mdContext, &((GC->ml_polyplaq[0][i]).comp[n1][n2][n3][n4]), sizeof(double complex)); }
					}
				}
			}
		}
	...
	for(i = 0; i < (param->d_space_vol); i++)
		...
		for(n1 = 0; n1 < NCOLOR; n1++)
			...
			for(n2 = 0; n2 < NCOLOR; n2++)
				...
				for(n3 = 0; n3 < NCOLOR; n3++)
					{
					for(n4 = 0; n4 < NCOLOR; n4++) { MD5_Update(&mdContext, &((GC->ml_polyplaqconn[0][i]).comp[n1][n2][n3][n4]), sizeof(double complex)); }
					}
				}
			}
		}
...
void end_clover_array(Gauge_Conf *GC, GParam const *const param)
	...
	for(r = 0; r < param->d_volume; r++)
		{
		for(i = 0; i < STDIM; i++) { free(GC->clover_array[r][i]); }
