...
inline static void gen_rand_array_c1o2(dsfmt_t *dsfmt, w128_t *array, long size);
inline static void gen_rand_array_c0o1(dsfmt_t *dsfmt, w128_t *array, long size);
inline static void gen_rand_array_o0c1(dsfmt_t *dsfmt, w128_t *array, long size);
inline static void gen_rand_array_o0o1(dsfmt_t *dsfmt, w128_t *array, long size);
...
inline static long idxof(long i) { return i ^ 1; }
#else
inline static long idxof(long i) { return i; }
#endif
 ...
 */
inline static void convert_c0o1(w128_t *w) { w->sd = _mm_add_pd(w->sd, sse2_double_m_one.d128); }
 ...
 * @param w 128bit stracture of double precision floating point numbers (I/O)
 */
inline static void convert_o0c1(w128_t *w) { w->sd = _mm_sub_pd(sse2_double_two.d128, w->sd); }
...
inline static void gen_rand_array_c1o2(dsfmt_t *dsfmt, w128_t *array, long size)
	...
	for(i = 1; i < DSFMT_N - DSFMT_POS1; i++) { do_recursion(&array[i], &dsfmt->status[i], &dsfmt->status[i + DSFMT_POS1], &lung); }
	for(; i < DSFMT_N; i++) { do_recursion(&array[i], &dsfmt->status[i], &array[i + DSFMT_POS1 - DSFMT_N], &lung); }
	for(; i < size - DSFMT_N; i++) { do_recursion(&array[i], &array[i - DSFMT_N], &array[i + DSFMT_POS1 - DSFMT_N], &lung); }
	for(j = 0; j < 2 * DSFMT_N - size; j++) { dsfmt->status[j] = array[j + size - DSFMT_N]; }
	for(; i < size; i++, j++)
		{
...
inline static void gen_rand_array_c0o1(dsfmt_t *dsfmt, w128_t *array, long size)
	...
	for(i = size - DSFMT_N; i < size; i++) { convert_c0o1(&array[i]); }
	dsfmt->status[DSFMT_N] = lung;
	}
...
inline static void gen_rand_array_o0o1(dsfmt_t *dsfmt, w128_t *array, long size)
	...
	for(i = size - DSFMT_N; i < size; i++) { convert_o0o1(&array[i]); }
	dsfmt->status[DSFMT_N] = lung;
	}
...
inline static void gen_rand_array_o0c1(dsfmt_t *dsfmt, w128_t *array, long size)
	...
	for(i = size - DSFMT_N; i < size; i++) { convert_o0c1(&array[i]); }
	dsfmt->status[DSFMT_N] = lung;
	}
...
static void initial_mask(dsfmt_t *dsfmt)
	...
	for(i = 0; i < DSFMT_N * 2; i++) { psfmt[i] = (psfmt[i] & DSFMT_LOW_MASK) | DSFMT_HIGH_CONST; }
	}
 ...
 */
long dsfmt_get_min_array_size(void) { return DSFMT_N64; }
...
void dsfmt_gen_rand_all(dsfmt_t *dsfmt)
	...
	for(i = 1; i < DSFMT_N - DSFMT_POS1; i++) { do_recursion(&dsfmt->status[i], &dsfmt->status[i], &dsfmt->status[i + DSFMT_POS1], &lung); }
	for(; i < DSFMT_N; i++) { do_recursion(&dsfmt->status[i], &dsfmt->status[i], &dsfmt->status[i + DSFMT_POS1 - DSFMT_N], &lung); }
	dsfmt->status[DSFMT_N] = lung;
	}
...
void dsfmt_chk_init_gen_rand(dsfmt_t *dsfmt, uint32_t seed, long mexp)
	...
	psfmt[idxof(0)] = seed;
	for(i = 1; i < (DSFMT_N + 1) * 4; i++) { psfmt[idxof(i)] = (uint32_t)1812433253UL * (psfmt[idxof(i - 1)] ^ (psfmt[idxof(i - 1)] >> 30)) + (uint32_t)i; }
	initial_mask(dsfmt);
...
void dsfmt_chk_init_by_array(dsfmt_t *dsfmt, uint32_t init_key[], long key_length, long mexp)
	...
	if(key_length + 1 > size) { count = key_length + 1; }
	else
		{
		count = size;
		}
